	...
	try {
		...
		if (isNotEmpty(migrations)) {
			for (const migration of migrations) {
				console.log(
					green(
						`Migration ${migration.name} has been run successfully!`
					)
				);
			}
		} else {
...
export async function generateMigration(options: IMigrationOptions) {
	if (!options.name) {
		console.log(
			yellow(
				'Migration name must be requried.Please specify migration name!'
			)
		);
		return;
	}
	...
	try {
		...
		if (upSqls.length) {
			...
			try {
				...
				console.log(
					black.bgRed('Error during migration generating files:')
				);
				console.error(error);
			}
		} else {
