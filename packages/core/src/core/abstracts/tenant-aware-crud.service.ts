...
import { BaseEntity, User } from '@database/entities/internal';
 ...
 */
export abstract class TenantAwareCrudService<T extends BaseEntity>
	...
	implements ICrudService<T>
{
	...
	private findConditionsWithEmployeeByUser(): FindOptionsWhere<T> {
		const profileId = RequestContext.currentProfileId();
		return (
			/**
			 * If employee has login & retrieve self data
			 */
			(
				isNotEmpty(profileId)
					? this.repository.metadata.hasColumnWithPropertyPath(
							'profileId'
					  )
						? {
								profile: {
