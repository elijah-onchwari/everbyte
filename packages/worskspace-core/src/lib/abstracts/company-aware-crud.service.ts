...
import { BaseEntity, User } from '@everbyte/data';
 ...
 */
export abstract class CompanyAwareCrudService<T extends BaseEntity> extends CrudService<T> implements ICrudService<T> {
	constructor(
	...
	*/
	...
	public async findOneOrFailByIdString(id: T['id'], options?: FindOneOptions<T>): Promise<ITryRequest<T>> {
		return await super.findOneOrFailByIdString(id, this.findOneWithCompany(options));
	}
