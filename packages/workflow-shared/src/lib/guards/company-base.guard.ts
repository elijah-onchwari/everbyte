...
import { RequestMethodEnum } from '@workflow/common';
...
export class CompanyBaseGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (
      headerCompanyId &&
      (rawHeaders.includes('company-id') || rawHeaders.includes('Company-Id'))
    ) {
        ...
        if ('companyId' in query) {
          ...
          if (isJson) {
            try {
              ...
              if ('findInput' in parse && 'companyId' in parse['findInput']) {
                const queryCompanyId = parse['findInput']['companyId'];
                isAuthorized = currentCompanyId === queryCompanyId;
              } else {
    ...
    if (!isAuthorized) {
      console.log('Unauthorized access blocked. CompanyId:', headerCompanyId);
    }
    return isAuthorized;
  }
}
